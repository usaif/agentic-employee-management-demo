...
def classify_intent(user_input: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"🔍 LLM: Using guardrail {GUARDRAIL_ID} version {GUARDRAIL_VERSION}", file=sys.stderr)
        ...
        if guardrail_action == 'INTERVENED':
            print(f"🛡️ GUARDRAIL BLOCKED THIS REQUEST!", file=sys.stderr)
            print(f"🛡️ Trace: {json.dumps(response_body.get('amazon-bedrock-trace', {}))}", file=sys.stderr)
        ...
        if content and len(content) > 0 and content[0].get('type') == 'text':
            ...
            try:
                ...
                valid_intents = {
                    "onboard", "authenticate", "view_self",
                    "view_employee", "update_employee",
                    "delete_employee", "unknown", "blocked"
                }
                
                intent = parsed.get("intent", "unknown")
                
                if intent not in valid_intents:
                    print(f"⚠️ Invalid intent '{intent}', defaulting to 'unknown'", file=sys.stderr)
        ...
        print(f"⚠️ No valid content in Claude response. Full response: {json.dumps(response_body)[:500]}", file=sys.stderr)
        return {"intent": "unknown"}
        ...
        if error_code == 'AccessDeniedException':
            print("💡 Ensure your IAM role has bedrock:InvokeModel and bedrock:ApplyGuardrail permissions", file=sys.stderr)
        elif error_code == 'ResourceNotFoundException':
            print("💡 Check: 1) Model access enabled in Bedrock console, 2) Guardrail ID is correct", file=sys.stderr)
...
"""
    ...
    try:
        ...
        if content and content[0].get('type') == 'text':
            ...
            try:
                parsed = json.loads(text_response)
                print(f"🧠 Claude reasoning: {parsed.get('reasoning', 'N/A')}", file=sys.stderr)
                return parsed
